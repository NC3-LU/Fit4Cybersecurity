...
-- 2. Clean the source DB and execute the data migrations to the new tables format:

CREATE EXTENSION IF NOT EXISTS dblink;

insert into survey_surveyquestionservicecategory select * from dblink('dbname=fit4contract_old port=5434', 'select sc.id, tr.text as label from survey_surveyquestionservicecategory as sc inner join survey_translationkey as tr on sc."titleKey" = tr.key and tr.lang = ''en''') AS t1(id int, label text);
...
insert into survey_surveysection select * from dblink('dbname=fit4contract_dev_old port=5434', 'select s.id, tr.text as label from survey_surveysection as s inner join survey_translationkey as tr on s."sectionTitleKey" = tr.key and tr.lang = ''en''') AS t1(id int, label text);
SELECT setval('survey_surveysection_id_seq', COALESCE((select max(id) from survey_surveysection)::bigint, 1));

insert into survey_surveyquestion select * from dblink('dbname=fit4contract_dev_old port=5434', 'select q.qindex AS id, tr.text as label, q.qtype, q.qindex, q.section_id, q.service_category_id, q."maxPoints", ''aindex'' answers_order, true is_active, '''' tooltip from survey_surveyquestion as q inner join survey_translationkey as tr on q."titleKey" = tr.key and tr.lang = ''en'' order by q.qindex') AS t1(id int, label text, qtype text, qindex int, section_id int, service_category_id int, "maxPoints" int, answers_order text, is_active boolean, tooltip text);
SELECT setval('survey_surveyquestion_id_seq', COALESCE((select max(id) from survey_surveyquestion)::bigint, 1));

insert into survey_surveyquestionanswer select * from dblink('dbname=fit4contract_dev_old port=5434', 'select qa.id, tr.text as label, qa.aindex / 10, q.qindex as question_id, qa."uniqueAnswer", qa.score, ''p'' as atype, 0 bonus_points, '''' "value", true is_active, '''' tooltip from survey_surveyquestionanswer as qa inner join survey_translationkey as tr on qa."answerKey" = tr.key and tr.lang = ''en'' inner join survey_surveyquestion as q on q.uuid = qa.question_id') AS t1(id int, label text, aindex int, question_id int, "uniqueAnswer" boolean, score int, atype text, bonus_points int, "value" text, is_active boolean, tooltip text);
SELECT setval('survey_surveyquestionanswer_id_seq', COALESCE((select max(id) from survey_surveyquestionanswer)::bigint, 1));

insert into survey_surveyuser select * from dblink('dbname=fit4contract_dev_old port=5434', 'select u.id, user_id, status, created_at, updated_at, u.choosen_lang as chosen_lang, q.qindex as current_question_id from survey_surveyuser as u inner join survey_surveyquestion as q on q.uuid = u.current_question_id') AS t1(id int, user_id uuid, status int, created_at date, updated_at date, chosen_lang text, current_question_id int);
SELECT setval('survey_surveyuser_id_seq', COALESCE((select max(id) from survey_surveyuser)::bigint, 1));

insert into survey_surveyuseranswer select * from dblink('dbname=fit4contract_dev_old port=5434', 'select id, uvalue, answer_id, user_id, '''' "content" from survey_surveyuseranswer') AS t1(id int, uvalue text, answer_id int, user_id int, "content" text);
SELECT setval('survey_surveyuseranswer_id_seq', COALESCE((select max(id) from survey_surveyuseranswer)::bigint, 1));

insert into survey_surveyuserfeedback select * from dblink('dbname=fit4contract_dev_old port=5434', 'select f.id, feedback, q.qindex as question_id, f.user_id from survey_surveyuserfeedback as f inner join survey_surveyquestion as q on q.uuid = f.question_id') AS t1(id int, feedback text, question_id int, user_id int);
SELECT setval('survey_surveyuserfeedback_id_seq', COALESCE((select max(id) from survey_surveyuserfeedback)::bigint, 1));

insert into survey_recommendations select * from dblink('dbname=fit4contract_dev_old port=5434', 'select r.id, tr.text as label, min_e_count, max_e_count, sector, "answerChosen", "forAnswer_id" from survey_recommendations as r inner join survey_translationkey as tr on r."textKey" = tr.key and tr.lang = ''en''') AS t1(id int, label text, min_e_count text, max_e_count text, sector text, "answerChosen" boolean, "forAnswer_id" int);
SELECT setval('survey_recommendations_id_seq', COALESCE((select max(id) from survey_recommendations)::bigint, 1));

insert into survey_recommendations_categories (recommendations_id, surveyquestionservicecategory_id) select * from dblink('dbname=fit4contract_dev_old port=5434', 'select r.id as recommendation_id, service_category_id from survey_recommendationcategory as rc inner join survey_recommendations as r on r."textKey" = rc.recommendation_key order by r.id') AS t1(recommendation_id int, service_category_id int);
SELECT setval('survey_recommendations_categories_id_seq', COALESCE((select max(id) from survey_recommendations_categories)::bigint, 1));

insert into survey_surveyuserquestionsequence select * from dblink('dbname=fit4contract_dev_old port=5434', 'select s.id, s.branch, s.level, s.index, has_been_answered, s.is_active, q.qindex as question_id, s.user_id from survey_surveyuserquestionsequence as s inner join survey_surveyquestion as q on q.uuid = s.question_id') AS t1(id int, branch int, "level" int, "index" int, has_been_answered boolean, is_active boolean, question_id int, user_id int);
...
-- Prepare the json for the next command, if not done or update with latest data.
...
-- 6. Finally, there are some questions updates, so we need to import questions on top of data move.
