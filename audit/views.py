...
def create_audit(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_audit.save()
            new_audit.auditbycompany_set.create(
                audit_company_id=request.user.company_set.all().get().id
            )
            new_audit.auditbyuser_set.create(user_id=request.user.id)
