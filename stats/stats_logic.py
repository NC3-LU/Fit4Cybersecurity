...
def get_finished_surveys_list(request):
    ...
    for question in SurveyQuestion.objects.all():
        ...
        max_evaluations_per_section[question.section.sectionTitleKey] += question.maxPoints
    ...
    for completed_survey_user in completed_surveys_users:
        survey_user_feedbacks = SurveyUserFeedback.objects.filter(user=completed_survey_user)
        ...
        surveys_users_results['survey_users'][user_id] = {
            'details': {
                ...
                "employees_number": [item[1] for item in COMPANY_SIZE if item[0] == completed_survey_user.e_count][0],
                "country_code": completed_survey_user.country_code,
                "language": completed_survey_user.choosen_lang,
                "survey_finish_date": completed_survey_user.updated_at.strftime(DEFAULT_DATE_FORMAT),
        ...
        user_answers = SurveyUserAnswer.objects.filter(user=completed_survey_user).order_by(
            "answer__question__qindex",
            "answer__aindex")
        for user_answer in user_answers:
            section_title_key = user_answer.answer.question.section.sectionTitleKey
            if section_title_key not in surveys_users_results['survey_users'][user_id]['sections']:
                surveys_users_results['survey_users'][user_id]['sections'][section_title_key] = {
                    'score': 0,
                    'questions': {},
            ...
            if question.titleKey not in surveys_users_results['survey_users'][user_id]['sections'][section_title_key]['questions']:
                has_feedback = 0
                if question.id in feedbacks_per_question:
                    has_feedback = 1
                surveys_users_results['survey_users'][user_id]['sections'][section_title_key]['questions'][question.titleKey] = {
                    'score': 0,
                    'answers': [],
                    'has_feedback': has_feedback,
                }

            surveys_users_results['survey_users'][user_id]['sections'][section_title_key]['questions'][question.titleKey]['answers'].append({
            ...
            if user_answer.uvalue > 0:
                ...
                surveys_users_results['survey_users'][user_id]['sections'][section_title_key]['questions'][question.titleKey]['score'] += round(
                    answer_score * 100 / user_answer.answer.question.maxPoints
                )
                surveys_users_results['survey_users'][user_id]['sections'][section_title_key]['score'] += round(
                    answer_score * 100 / max_evaluations_per_section[section_title_key]
                )

        surveys_users_results['survey_users'][user_id]['details']['total_score'] = round(
            total_points_number * 100 / total_questions_score
        )
